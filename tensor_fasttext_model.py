...
# datetime:18-12-16
# function:
# 参考网址：https://github.com/brightmart/text_classification/tree/master/a01_FastText
...
class fastTextB:
    ...
    def loss11(self,l2_lambda=0.01): #0.0001-->0.001
    ...
    def loss(self,l2_lambda=0.01): #0.0001-->0.001
        ...
        # 需要注意的一点区别：
        # （1）sparse_softmax_cross_entropy_with_logits：labels是int类型
        # （2）softmax_cross_entropy_with_logits的差别：labels是one-hot类型。
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            # input_y = np.array([1, 0, 1, 0, 1, 0, 1, 1, 0, 1],dtype=np.int32)  # np.zeros((batch_size),dtype=np.int32) #[None, self.sequence_length]
            input_x=np.random.rand(batch_size,sequence_length) #[None, self.sequence_length]
            input_y = np.random.randint(0,2,size=batch_size)
